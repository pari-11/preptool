...
export type BulkSolvedResult = {
  marked: { leetcodeId: number; title: string }[];
  // Problems in several stages: marked in the earliest stage only.
  multiStage: { leetcodeId: number; title: string; markedStage: string; otherStages: string[] }[];
  alreadySolved: number[];
  notFound: number[];
  invalid: string[];
};

// Marks LeetCode IDs solved with an unknown date (a ReviewLog row with solved_at = null).
// Idempotent: a problem that is already solved is skipped and gets no extra row.
export async function markSolvedByLeetcodeIds(rawInput: string): Promise<BulkSolvedResult> {
  const result: BulkSolvedResult = {
    marked: [],
    multiStage: [],
    alreadySolved: [],
    notFound: [],
  ...
  const ids: number[] = [];
  for (const token of rawInput.split(/[\s,;]+/).filter(Boolean)) {
    if (!/^\d+$/.test(token)) {
  ...
  for (const leetcodeId of ids) {
    await prisma.$transaction(async (tx) => {
      const problem = await tx.problem.findUnique({
        where: { leetcode_id: leetcodeId },
        include: { stages: { include: { stage: true }, orderBy: { stage: { order: 'asc' } } } },
      });
      if (!problem) {
        result.notFound.push(leetcodeId);
        return;
      }
      if (problem.is_solved) {
        result.alreadySolved.push(leetcodeId);
        return;
      }

      const earliest = problem.stages[0];
      await recordSolve(tx, problem.id, earliest?.stage_id ?? null, null);

      if (problem.stages.length > 1) {
        result.multiStage.push({
          leetcodeId,
          title: problem.title,
          markedStage: earliest.stage.stage_label,
          otherStages: problem.stages.slice(1).map((s) => s.stage.stage_label),
        });
      } else {
        result.marked.push({ leetcodeId, title: problem.title });
      }
    });
  }
