...
| roadmap_order | Int | global sequence position |
| tier | Enum(Core, Supp, Stretch) | |
...
- [ ] `prisma/schema.prisma` defines Company, Stage, Problem, Resource, and both join relations.
...
- [ ] A throwaway script (deleted after verification, not committed) can create one Company, one Stage, one Problem linked to that Company, and one Resource linked to that same Company — confirming the many-to-many relations work in both directions.
